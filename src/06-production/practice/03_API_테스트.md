...
- 원본 파일: `06-프로덕션으로-가는-길/실습/03_API_테스트.py`
...
# 1단계는 서버가 살아 있는지 가장 가벼운 health 엔드포인트로 확인합니다.
print("=" * 60)
...
# 2단계는 루트 응답으로 제공 가능한 엔드포인트 목록과 버전을 확인합니다.
print("\n" + "=" * 60)
print("2. Root")
print("=" * 60)
...
# 3단계는 /info로 메타데이터(버전/모델 타입/성능)가 정상 노출되는지 확인합니다.
print("\n" + "=" * 60)
print("3. Model Info")
print("=" * 60)
...
# 4단계는 단건 예측이 정상 동작하는지 대표 샘플로 검사합니다.
print("\n" + "=" * 60)
print("4. 단일 예측")
print("=" * 60)
...
# 5단계는 여러 샘플을 한 번에 보내는 배치 API의 형식/성능을 점검합니다.
print("\n" + "=" * 60)
...
# 6단계는 의도적으로 잘못된 요청을 보내 검증 로직(422)이 동작하는지 확인합니다.
print("\n" + "=" * 60)
...
# 7단계는 간단한 반복 호출로 평균 지연 시간과 처리량(req/s)을 대략 측정합니다.
print("\n" + "=" * 60)
